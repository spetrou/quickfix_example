...
void Application::onMessage( const FIX50::NewOrderSingle& message, const FIX::SessionID& )
{
...
void Application::onMessage( const FIX50::OrderCancelRequest& message, const FIX::SessionID& )
{
...
void Application::onMessage( const FIX50::MarketDataRequest& message, const FIX::SessionID& )
{
  FIX::MDReqID mdReqID;
...
void Application::updateOrder( const Order& order, char status )
{
  ...
  FIX50::ExecutionReport fixOrder
  ...
  const FIX::Side& side, const std::string& message )
{
  ...
  ( FIX::OrderID ( clOrdID.getValue() ),
    ...
    side, FIX::LeavesQty( 0 ), FIX::CumQty( 0 ) );
